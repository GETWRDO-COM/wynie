...
const MegaMenu = ({ open, onClose, kind }) => {
  if (!open) return null;
  ...
  return (
    <div className="absolute left-1/2 -translate-x-1/2 mt-2 w-[720px] rounded-2xl border border-white/10 bg-[#0b0f14]/95 backdrop-blur-xl shadow-2xl p-6 z-50">
...
const NavBar = ({ activeTab, setActiveTab, user, onSettings, onLogout }) => {
  ...
  return (
    <header className="sticky top-0 z-50">
      <div className="bg-[#070a11]/98 border-b border-white/10">
              ...
              <div className="flex items-center gap-1 p-1 rounded-xl border border-white/10 bg-white/5">
                {TABS.map((t) => (
                  <button
                    ...
                    className={`relative px-3 py-1.5 rounded-lg text-sm transition-colors ${activeTab === t.id ? 'text-white bg-white/10 border border-white/10' : 'text-gray-300 hover:text-white hover:bg-white/5'}`}
                  >
                    {t.label}
                  </button>
                ))}
