...
function parseInTZ(now, timeZone) { /* unchanged helpers */ const fmt = new Intl.DateTimeFormat('en-GB', { timeZone, year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' }).format(now); const [datePart, timePart] = fmt.split(', '); const [d, m, y] = datePart.split('/').map(s => parseInt(s, 10)); const [hh, mm, ss] = timePart.split(':').map(s => parseInt(s, 10)); return { y, m, d, hh, mm, ss }; }
...
function nextNYSEOpenClose(now){ const tz='America/New_York'; const wd=new Intl.DateTimeFormat('en-US',{ timeZone: tz, weekday:'short'}).format(now); const isWeekend = wd==='Sat'||wd==='Sun'; const toOpen=secondsUntil(now,tz,9,30); const halfDay = isHalfDay(now); const closeHour = halfDay ? 13 : 16; const toClose=secondsUntil(now,tz,closeHour,0); const holidayName=nyseHolidayToday(now) || (halfDay ? 'Early Close' : null);
  ...
  let days=1; while(true){ const candidate=new Date(now.getTime()); candidate.setUTCDate(candidate.getUTCDate()+days); const w=new Intl.DateTimeFormat('en-US',{ timeZone: tz, weekday:'short'}).format(candidate); const weekend = w==='Sat'||w==='Sun'; if(!weekend && !nyseHolidayToday(candidate)) break; days+=1; }
