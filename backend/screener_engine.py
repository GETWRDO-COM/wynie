...
COMPUTED_FIELDS = {"sma20":("daily", lambda m:m.get("sma20")),"sma50":("daily", lambda m:m.get("sma50")),"sma200":("daily", lambda m:m.get("sma200")),"ema8":("daily", lambda m:m.get("ema8")),"ema21":("daily", lambda m:m.get("ema21")),"ema50":("daily", lambda m:m.get("ema50")),"rsi14":("daily", lambda m:m.get("rsi14")),"atr14":("daily", lambda m:m.get("atr14")),"bb_bw":("daily", lambda m:m.get("bb_bw")),"stoch_k":("daily", lambda m:m.get("stoch_k")),"avgVol20d":("daily", lambda m:m.get("avgVol20d")),"runRate20d":("daily", lambda m:m.get("runRate20d")),"relVol":("daily", lambda m:m.get("relVol")),"pct_to_hi52":("daily", lambda m:m.get("pct_to_hi52")),"pct_above_lo52":("daily", lambda m:m.get("pct_above_lo52"))}

FUND_FIELDS = { # map registry id -> (source key path)
...
def get_fundamentals(fin: FinnhubClient, symbol: str) -> Dict[str, Any]:
    ...
    if cached is not None: return cached
    prof = fin.company_profile(symbol)
    met = fin.metrics(symbol)
    data = {"profile": prof, "metrics": met}
    CACHE.set(CACHE.fund, key, data, 1800)  # 30m
    return data
